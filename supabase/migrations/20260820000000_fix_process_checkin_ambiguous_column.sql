-- Fix: process_checkin()'s RETURNS TABLE(..., rate_limited boolean, ...)
-- implicitly declares a PL/pgSQL variable named rate_limited, which
-- collides with checkins.rate_limited inside the 4-hour rate-limit lookup
-- query, making that column reference ambiguous. Discovered while
-- verifying the FoodieMon -> Foodlings rebrand (unrelated to the rename
-- itself — this bug predates it and meant check-ins never actually
-- completed). Fix: qualify the column with the table name.
...
begin
  ...
  -- checkins.rate_limited is qualified below because RETURNS TABLE's
  -- own "rate_limited" output column otherwise makes this ambiguous.
